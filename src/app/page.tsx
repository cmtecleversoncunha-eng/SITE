...
import { getFeaturedProducts, getCompanySettings, getSiteImages, getKitInicianteSettings } from '@/lib/data';
...
export default async function Home() {
  ...
  // Buscar imagens e kit iniciante do editor
  const siteImages = await getSiteImages();
  const kitSettings = await getKitInicianteSettings();
            ...
            <div className="flex flex-col items-center gap-8 px-6 py-16 md:flex-row">
              <div className="flex-1 text-center md:text-left">
                ...
                <p className="mt-4 text-lg text-primary-foreground/80">
                  {kitSettings.subtitle}
                </p>
                <Button asChild size="lg" className="mt-8 bg-accent text-accent-foreground hover:bg-accent/90 font-headline font-bold">
                  <Link href={kitSettings.redirectUrl}>
                    {kitSettings.buttonText}
                  </Link>
                </Button>
              </div>
              <div className="flex-1">
                <Image
                  src={siteImages.kitImage}
